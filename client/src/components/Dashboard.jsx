...
const Dashboard = () => {
    ...
    const run = async () => {
        try {
            ...
            console.error(new Date().toLocaleString(), 'Error running code:', error.response.data, new Date().toLocaleString().split(",")[1]);
                    ...
                    <div className='absolute z-40 border-2 border-green-400 terminal bg-gray-950 bottom-0 h-96 overflow-y-scroll'>
                        ...
                        {codeResHistory.map((prevCodeRes, index) => (
                            <div key={index} className="p-4 bg-gray-900 rounded-lg mb-2 p-2">
                                <div className={`m-5 font-mono ${prevCodeRes.startsWith("Out") ? "text-green-600" : "text-red-600"}`}>
                                    <pre>
                                        <span className='text-gray-400'>{new Date().toLocaleString().split(",")[1]}</span> {prevCodeRes}
                                    </pre>
                                </div>
                            </div>
                        ))}
